...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM ''D:\SQLImport\CLV\clv_crm_pipe.csv'
        WITH (
            FIRSTROW     = 2,
            FIELDTERMINATOR = '|',
            ROWTERMINATOR = '0x0a',
        ...
        SET @start_time = GETDATE();
        PRINT '>> Truncating Table: raw.orders '
        TRUNCATE TABLE raw.orders

        PRINT '>> Inserting Data Into: raw.orders'
        BULK INSERT raw.orders
        FROM 'D:\SQLImport\CLV\clv_orders_pipe.csv'
        WITH (
            FIRSTROW     = 2,
            FIELDTERMINATOR = '|',
            ROWTERMINATOR = '0x0a',
...
-- 3# Load order items
        ...
        SET @start_time = GETDATE();
        PRINT '>> Truncating Table: raw.order_items'
        TRUNCATE TABLE raw.order_items

        PRINT '>> Inserting Data Into: raw.order_items'
        BULK INSERT raw.order_items
        FROM 'D:\SQLImport\CLV\clv_items_pipe.csv'
        WITH (
            FIRSTROW     = 2,
            FIELDTERMINATOR = '|',
            ROWTERMINATOR = '0x0a',
